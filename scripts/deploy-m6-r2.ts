/**
 * deploy-m6-r2.ts — Deploy M6 r2 AirAccount Factory to Sepolia via viem
 *
 * M6 r2 changes vs M6 r1:
 ...
 *   TOKEN_PROFILE=conservative pnpm tsx scripts/deploy-m6-r2.ts
 *
 * After deploy, update .env.sepolia:
 *   AIRACCOUNT_M6_R2_FACTORY=<factory address>
 *   AIRACCOUNT_M6_R2_IMPL=<implementation address (read from factory.implementation())>
 */
...
async function main() {
  console.log("=== Deploy M6 r2 AirAccount Factory to Sepolia ===\n");
  ...
  // ─── Deploy Factory ───────────────────────────────────────────────
  console.log("Deploying AAStarAirAccountFactoryV7 (M6 r2)...");
  ...
  // ─── Summary ──────────────────────────────────────────────────────
  console.log("=== M6 r2 Factory Deployment Summary ===");
  console.log(`AIRACCOUNT_M6_R2_FACTORY=${factoryAddress}`);
  console.log(`AIRACCOUNT_M6_R2_IMPL=${implAddress}`);
  console.log();
