/**
 * deploy-v0.32.0.ts — Deploy for v0.32.0 (CC-116 committee-off fail-closed gate on top of the v0.31.0
 ...
 *
 * v0.32.0 vs v0.31.0 (v0.31.0 was deployed to Sepolia; this is the next non-upgradable stack):
 ...
 *
 * ── COMMITTEE VALIDATOR: mounted at algId 0x01 of the NEW v0.32.0 router (a fresh set-once router). This
 *    does NOT edit the v0.29.0/v0.31.0 production routers. v0.32.0 is a parallel new stack.
...
// mounted at algId 0x01 of the NEW v0.32.0 router (fresh set-once router; v0.29.0/v0.31.0 routers are
...
// grown v0.32.0 bytecode (feedback_impl_gas_limit).
async function dryRun() {
  console.log("\n=== v0.32.0 DRY RUN — no transactions sent ===");
...
async function main() {
  ...
  const committeeValidator = getAddress(DVT_COMMITTEE_VALIDATOR);
  console.log(`\n=== v0.32.0 deploy — Sepolia (CC-116 committee-off fail-closed + CC-102 gov + CC-98 committee) ===`);
  ...
  console.log(`AIRACCOUNT_V0320_WEBAUTHN_LIB=${webAuthnLib}`);
  console.log(`AIRACCOUNT_V0320_COMMITTEE_BLS_LIB=${committeeLib}`);
  console.log(`AIRACCOUNT_V0320_IMPL=${impl}`);
  console.log(`AIRACCOUNT_V0320_EXTENSION=${extension}`);
  console.log(`AIRACCOUNT_V0320_FACTORY=${factory}`);
  console.log(`AIRACCOUNT_V0320_AGENT_REGISTRY=${agentRegistry}`);
  console.log(`AIRACCOUNT_V0320_VALIDATOR_ROUTER=${router}`);
  console.log(`AIRACCOUNT_V0320_COMMITTEE_VALIDATOR=${committeeValidator}`);
  console.log(`AIRACCOUNT_V0320_SESSION_KEY_VALIDATOR=${sessionValidator}`);
  console.log(`AIRACCOUNT_V0320_FORCE_EXIT_MODULE=${forceExitModule}`);
  console.log(`AIRACCOUNT_V0320_DELEGATE=${delegate}`);
  console.log(`AIRACCOUNT_V0320_PARSER_REGISTRY=${parserRegistry}`);
}
