/**
 * e2e-v0.25.0-bundler.ts — REAL on-chain UserOp E2E for v0.25.0 (CRITICAL-1) via Pimlico bundler.
 *
 * Proves, through the actual ERC-4337 validateUserOp→execute path (not just view checks):
 *   B1  createAccountWithDefaults (guard-enabled, dailyLimit>0) on the v0.25.0 factory
 ...
 *       On v0.24.0 the raw-65 M1 fallback accepted this; v0.25.0 removed it, so validateUserOp returns
 ...
 *
 * Signature: EXPLICIT [0x02][r][s][v] = 66 bytes (v0.25.0 requires it; raw-65 rejected).
 * Uses PRIVATE_KEY_ANNI (owner), PRIVATE_KEY_JASON + PRIVATE_KEY_BOB (guardians), Pimlico bundler.
 *
 * Run: pnpm tsx scripts/e2e-v0.25.0-bundler.ts
 */
...
const FACTORY = getAddress(process.env.AIRACCOUNT_V0250_FACTORY!);
...
// EXPLICIT 0x02 ECDSA sig (66 bytes) over EIP-191(userOpHash) — v0.25.0 requires the prefix.
...
async function main() {
  console.log(`\n=== v0.25.0 REAL UserOp E2E (Pimlico) — factory ${FACTORY} ===\n`);
  ...
  console.log("All REAL UserOp checks PASSED — v0.25.0 verified end-to-end on-chain.");
}
