/**
 * deploy-v0.20.ts — Full v0.20 Sepolia deploy (WS-A/B/C/E/G changed singletons).
 *
 * Unlike beta.4 (which only redeployed impl/factory/delegate/agentRegistry and REUSED the
 * BLS/validator/session/forceexit singletons), v0.20 changed those singletons, so this script
 ...
 *   5. ForceExitModule              (no-arg; v0.20.0)
 ...
 *
 * IDEMPOTENT-ish: every AIRACCOUNT_V020_* env var that is already set is REUSED (skip-deploy).
 ...
 *
 * THIS SCRIPT DOES NOT BROADCAST UNLESS RUN. Build/dry-validate is in deploy-v0.20.check.ts.
 *
 * Usage: pnpm tsx scripts/deploy-v0.20.ts
 */
...
async function dryCheck() {
  console.log("=== DRY-CHECK (no broadcast) — v0.20 deploy ===");
...
async function main() {
  ...
  console.log("=== Deploy AirAccount v0.20 (Sepolia) ===");
  console.log(`Deployer:   ${deployer.address}`);
  ...
  // P-256 (passkey) verification is cheap with the P256VERIFY precompile at 0x100 (~6,900 gas on L1
  // EIP-7951, ~3,450 on OP-Stack RIP-7212). Without it, ALL P256 sites (shared Solady primitive since
  // #191) fall back to the canonical Solidity verifier at 0x000000000000D01eA45F9eFD5c54f037Fa57Ea1a
  // (~300k gas, deployed on most chains incl. Sepolia — `cast code`); P256 only fails where NEITHER the
  // precompile NOR that verifier is present. This probe warns only about the (cheap-vs-~300k) cost tier.
  // Probe with a valid
  // vector (returns 1 iff live) and WARN before spending deploy gas on a chain that lacks it.
  {
    ...
    if (has7212) {
      ...
      console.log("  → P-256 falls back to the ~300k-gas Solidity verifier (0x…Ea1a) where it is deployed; fails only where that verifier is also absent.");
  ...
  const validatorRouter = await deployOrReuse("AIRACCOUNT_V020_VALIDATOR_ROUTER", "AAStarValidator", "AAStarValidator", [], GAS.validatorRouter);

  console.log("[3/10] AAStarBLSAggregator(blsAlgorithm, entryPoint)...");
  const blsAggregator = await deployOrReuse("AIRACCOUNT_V020_BLS_AGGREGATOR", "AAStarBLSAggregator", "AAStarBLSAggregator", [blsAlgorithm, ENTRYPOINT], GAS.blsAggregator);

  console.log("[4/10] SessionKeyValidator...");
  const sessionKeyValidator = await deployOrReuse("AIRACCOUNT_V020_SESSION_KEY_VALIDATOR", "SessionKeyValidator", "SessionKeyValidator", [], GAS.sessionKeyValidator);

  console.log("[5/10] ForceExitModule...");
  const forceExitModule = await deployOrReuse("AIRACCOUNT_V020_FORCE_EXIT_MODULE", "ForceExitModule", "ForceExitModule", [], GAS.forceExitModule);

  console.log("[6/10] AirAccountDelegate...");
  const delegate = await deployOrReuse("AIRACCOUNT_V020_DELEGATE", "AirAccountDelegate", "AirAccountDelegate", [], GAS.delegate);

  console.log("[7/10] CalldataParserRegistry...");
  const parserRegistry = await deployOrReuse("AIRACCOUNT_V020_PARSER_REGISTRY", "CalldataParserRegistry", "CalldataParserRegistry", [], GAS.parserRegistry);

  console.log("[8/10] AAStarAirAccountV7 (impl + Extension)...");
  const impl = await deployOrReuse("AIRACCOUNT_V020_IMPL", "AAStarAirAccountV7", "AAStarAirAccountV7", [], GAS.impl);

  console.log("[9/10] AAStarAirAccountFactoryV7(impl, entryPoint, community, [], [])...");
  const factory = await deployOrReuse("AIRACCOUNT_V020_FACTORY", "AAStarAirAccountFactoryV7", "AAStarAirAccountFactoryV7", [impl, ENTRYPOINT, COMMUNITY, [], []], GAS.factory);

  console.log("[10/10] AgentRegistry...");
  const agentRegistry = await deployOrReuse("AIRACCOUNT_V020_AGENT_REGISTRY", "AgentRegistry", "AgentRegistry", [], GAS.agentRegistry);
  ...
  console.log("\n=== v0.20 Deployment Summary — append to .env.sepolia ===");
  console.log(`AIRACCOUNT_V020_FACTORY=${factory}`);
  console.log(`AIRACCOUNT_V020_IMPL=${impl}`);
  console.log(`AIRACCOUNT_V020_SESSION_KEY_VALIDATOR=${sessionKeyValidator}`);
  console.log(`AIRACCOUNT_V020_FORCE_EXIT_MODULE=${forceExitModule}`);
  console.log(`AIRACCOUNT_V020_VALIDATOR_ROUTER=${validatorRouter}`);
  console.log(`AIRACCOUNT_V020_EXTENSION=${extension}`);
  console.log(`AIRACCOUNT_V020_AGENT_REGISTRY=${agentRegistry}`);
  console.log(`AIRACCOUNT_V020_BLS_ALGORITHM=${blsAlgorithm}`);
  console.log(`AIRACCOUNT_V020_BLS_AGGREGATOR=${blsAggregator}`);
  console.log(`AIRACCOUNT_V020_DELEGATE=${delegate}`);
  console.log(`AIRACCOUNT_V020_PARSER_REGISTRY=${parserRegistry}`);
