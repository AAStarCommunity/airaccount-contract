/**
 * deploy-v0.21.0.ts — Deploy for v0.21.0 (WebAuthn-native cumulative algIds 0x09/0x0a, issue #147 / PR #148).
 *
 * What changed vs v0.20.3:
 *   - AAStarAirAccountBase: new ALG_CUMULATIVE_T2_WA (0x09) + ALG_CUMULATIVE_T3_WA (0x0a)
 *     - _base64UrlEncode32() internal pure helper
 *     - _verifyWebAuthnOwnerSig() — WebAuthn assertion verification against owner P256 key
 *     - _validateCumulativeTier2WA() — WebAuthn + BLS DVT
 *     - _validateCumulativeTier3WA() — WebAuthn + BLS DVT + Guardian ECDSA
 *     - Overflow-safe ABI pre-validation in _verifyWebAuthnOwnerSig (F1 fix)
 *   - AlgTierLib: 0x09 → tier 2, 0x0a → tier 3
 *   - AAStarAirAccountFactoryV7: algIds array expanded 8→10 (added 0x09, 0x0a)
 *   - ACCOUNT_VERSION bumped to "0.21.0", FACTORY_VERSION to "0.21.0"
 *
 ...
 *   AAStarAirAccountV7 (impl)   — AAStarAirAccountBase changed; Extension embedded in ctor
 *   AAStarAirAccountFactoryV7   — algIds array changed; must point to new impl
 ...
 *
 * Output env keys: AIRACCOUNT_V0210_*
 *
 * Usage: pnpm tsx scripts/deploy-v0.21.0.ts
 */
...
// ── Reused v0.20.3 singletons (must be set in .env.sepolia) ──────────────────
function v0203(key: string): Address {
  const v = process.env[`AIRACCOUNT_V0203_${key}`];
  if (!v) throw new Error(`AIRACCOUNT_V0203_${key} not set in .env.sepolia — run deploy-v0.20.3.ts first`);
  return getAddress(v);
}
...
async function main() {
  ...
  console.log(`\n=== v0.21.0 deploy — Sepolia ===`);
  console.log(`Deployer: ${deployer.address}\n`);

  // ── Reused v0.20.3 singletons ──────────────────────────────────────────
  const blsAlgorithm        = v0203("BLS_ALGORITHM");
  const validatorRouter     = v0203("VALIDATOR_ROUTER");
  const blsAggregator       = v0203("BLS_AGGREGATOR");
  const sessionKeyValidator = v0203("SESSION_KEY_VALIDATOR");
  const forceExitModule     = v0203("FORCE_EXIT_MODULE");
  const delegate            = v0203("DELEGATE");
  const parserRegistry      = v0203("PARSER_REGISTRY");
  console.log("Reusing v0.20.3 singletons:");
  ...
  // ── Deploy: factory (algIds array now includes 0x09 + 0x0a) ──────────────
  console.log("[2/3] AAStarAirAccountFactoryV7 (10 algIds, pointing to new impl)...");
  ...
  if (ver !== "0.21.0") throw new Error(`Version mismatch: expected "0.21.0", got "${ver}"`);
  console.log("  v0.21.0 confirmed\n");

  // ── Summary ───────────────────────────────────────────────────────────────
  console.log("=== v0.21.0 Deployment Complete ===");
  console.log("Append to .env.sepolia:\n");
  console.log(`AIRACCOUNT_V0210_IMPL=${impl}`);
  console.log(`AIRACCOUNT_V0210_EXTENSION=${extension}`);
  console.log(`AIRACCOUNT_V0210_FACTORY=${factory}`);
  console.log(`AIRACCOUNT_V0210_AGENT_REGISTRY=${agentRegistry}`);
  console.log(`AIRACCOUNT_V0210_BLS_ALGORITHM=${blsAlgorithm}`);
  console.log(`AIRACCOUNT_V0210_BLS_AGGREGATOR=${blsAggregator}`);
  console.log(`AIRACCOUNT_V0210_VALIDATOR_ROUTER=${validatorRouter}`);
  console.log(`AIRACCOUNT_V0210_SESSION_KEY_VALIDATOR=${sessionKeyValidator}`);
  console.log(`AIRACCOUNT_V0210_FORCE_EXIT_MODULE=${forceExitModule}`);
  console.log(`AIRACCOUNT_V0210_DELEGATE=${delegate}`);
  console.log(`AIRACCOUNT_V0210_PARSER_REGISTRY=${parserRegistry}`);
}
