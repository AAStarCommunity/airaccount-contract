 ...
 *
 * Changes from r5:
 *   - AAStarAirAccountV7: SEL_ON_INSTALL/SEL_ON_UNINSTALL named constants (bytecode equivalent)
 *   - TierGuardHook: ALG_BLS/ALG_COMBINED_T1/ALG_SESSION_KEY named constants (bytecode equivalent)
 *   - AgentSessionKeyValidator: extracted _containsAddress/_containsSelector helpers (behavior equivalent)
 *   - AAStarAirAccountFactoryV7: guardian1 != guardian2 distinct check (new safety guard)
 *
 * Deploys:
 *   1. AirAccountCompositeValidator
 *   2. TierGuardHook
 *   3. AgentSessionKeyValidator
 *   4. AAStarAirAccountFactoryV7 (r6 implementation)
 *   5. Test account via factory (salt=800, fresh account for E2E)
 *   6. Funds account with 0.05 ETH
 *
 * Usage:
 *   pnpm tsx scripts/deploy-m7-r6.ts
 *
...
const SALT = 800n; // M7 r6 — fresh account for clean E2E
...
async function main() {
  ...
  console.log("=== Deploy AirAccount M7 r6 (simplify + guardian distinct fix) to Sepolia ===\n");
  ...
  console.log("\n[4/6] Deploy AAStarAirAccountFactoryV7 (M7 r6)...");
  ...
  // ─── Summary ─────────────────────────────────────────────────────────────
  console.log("\n═══════════════════════════════════════════════════════════════");
  console.log(" M7 r6 Deployment Complete");
  console.log("═══════════════════════════════════════════════════════════════");
  console.log("\nAdd to .env.sepolia:");
  console.log(`AIRACCOUNT_M7_R6_FACTORY=${factoryAddr}`);
  console.log(`AIRACCOUNT_M7_R6_IMPL=${implAddr}`);
  console.log(`AIRACCOUNT_M7_R6_ACCOUNT=${predictedAddr}`);
  console.log(`AIRACCOUNT_M7_R6_COMPOSITE_VALIDATOR=${compositeValidatorAddr}`);
  console.log(`AIRACCOUNT_M7_R6_TIER_GUARD_HOOK=${tierGuardHookAddr}`);
  console.log(`AIRACCOUNT_M7_R6_AGENT_SESSION_VALIDATOR=${agentSessionValidatorAddr}`);
