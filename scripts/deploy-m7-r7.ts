/**
 * deploy-m7-r7.ts — Deploy AirAccount M7 r7 (community guardian fix) to Sepolia
 *
 * Changes from r6:
 *   - AAStarAirAccountFactoryV7: community guardian = 0x51eDf11fDb0A4F66220eFb8efA54Eca77232E114
 *     (Safe multisig on Sepolia/mainnet/OP mainnet; r6 used address(0) by mistake)
 *
 * Deploys:
 *   1. AirAccountCompositeValidator
 *   2. TierGuardHook
 *   3. AgentSessionKeyValidator
 *   4. AAStarAirAccountFactoryV7 (r7 implementation)
 *   5. Test account via factory (salt=900, fresh account for E2E)
 *   6. Funds account with 0.05 ETH
 *
 * Usage:
 *   pnpm tsx scripts/deploy-m7-r7.ts
 *
...
const SALT = 900n; // M7 r7 — fresh account for clean E2E
...
async function main() {
  ...
  console.log("=== Deploy AirAccount M7 r7 (community guardian fix) to Sepolia ===\n");
  ...
  console.log("\n[4/6] Deploy AAStarAirAccountFactoryV7 (M7 r7)...");
  ...
  // ─── Summary ─────────────────────────────────────────────────────────────
  console.log("\n═══════════════════════════════════════════════════════════════");
  console.log(" M7 r7 Deployment Complete");
  console.log("═══════════════════════════════════════════════════════════════");
  console.log("\nAdd to .env.sepolia:");
  console.log(`AIRACCOUNT_M7_R7_FACTORY=${factoryAddr}`);
  console.log(`AIRACCOUNT_M7_R7_IMPL=${implAddr}`);
  console.log(`AIRACCOUNT_M7_R7_ACCOUNT=${predictedAddr}`);
  console.log(`AIRACCOUNT_M7_R7_COMPOSITE_VALIDATOR=${compositeValidatorAddr}`);
  console.log(`AIRACCOUNT_M7_R7_TIER_GUARD_HOOK=${tierGuardHookAddr}`);
  console.log(`AIRACCOUNT_M7_R7_AGENT_SESSION_VALIDATOR=${agentSessionValidatorAddr}`);
