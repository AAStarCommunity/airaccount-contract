/**
 * deploy-v0.23.0.ts — Deploy for v0.23.0 (isValidOwnerAuth owner-authorization view, issue #159).
 *
 * What changed vs v0.22.0:
 *   - AirAccountExtension: added `isValidOwnerAuth(bytes32 userOpHash, bytes calldata ownerAuth)
 *     external view returns (bytes4)` — the DVT/relayer single source of truth for owner
 *     authorization (ECDSA personal_sign OR owner WebAuthn passkey), reached via the account's
 *     fallback -> delegatecall. Magic 0xa0cf00cf; fail-closed. Unblocks device-passkey Tier-3 DVT.
 *   - No account/factory API change — createAccount/getAddress identical to v0.22.0.
 *   - ACCOUNT_VERSION / FACTORY_VERSION bumped to "0.23.0".
 *
 * Because the view lives in AirAccountExtension (deployed by the impl constructor), redeploying the
 * impl automatically deploys the new Extension. Factory must point to the new impl; AgentRegistry is
 * fresh (bindFactory is set-once).
 *
 * Reused from v0.22.0 (unchanged bytecode):
 ...
 *   AAStarAirAccountV7 (impl)   — new Extension (isValidOwnerAuth) + version bump
 *   AAStarAirAccountFactoryV7   — version bump; must point to new impl
 *   AgentRegistry               — fresh instance for the new factory
 *
 * Wiring (post-deploy):
 *   W1. agentRegistry.bindFactory(factory)
 *   W2. factory.setAgentRegistry(agentRegistry)
 *
 * Output env keys: AIRACCOUNT_V0230_*
 *
 * Usage: pnpm tsx scripts/deploy-v0.23.0.ts
 */
...
function v0220(key: string): Address {
  const v = process.env[`AIRACCOUNT_V0220_${key}`];
  if (!v) throw new Error(`AIRACCOUNT_V0220_${key} not set in .env.sepolia — run deploy-v0.22.0.ts first`);
  return getAddress(v);
}
...
async function main() {
  ...
  console.log(`\n=== v0.23.0 deploy — Sepolia ===`);
  console.log(`Deployer: ${deployer.address}\n`);

  // ── Reused v0.22.0 singletons ──────────────────────────────────────────────
  const blsAlgorithm        = v0220("BLS_ALGORITHM");
  const validatorRouter     = v0220("VALIDATOR_ROUTER");
  const blsAggregator       = v0220("BLS_AGGREGATOR");
  const sessionKeyValidator = v0220("SESSION_KEY_VALIDATOR");
  const forceExitModule     = v0220("FORCE_EXIT_MODULE");
  const delegate            = v0220("DELEGATE");
  const parserRegistry      = v0220("PARSER_REGISTRY");
  console.log("Reusing v0.22.0 singletons:");
  ...
  // ── Deploy: impl (validatorRouter baked in; ctor pushes gas ~10M → 15M) ──────
  console.log("[1/3] AAStarAirAccountV7 (impl, new Extension w/ isValidOwnerAuth)...");
  ...
  // ── Deploy: factory (points to new impl; same constructor as v0.22.0) ────────
  console.log("\n[2/3] AAStarAirAccountFactoryV7 (pointing to new impl)...");
  ...
  // ── Summary ───────────────────────────────────────────────────────────────────
  console.log("=== v0.23.0 Deployment Complete ===");
  console.log("Append to .env.sepolia:\n");
  console.log(`AIRACCOUNT_V0230_IMPL=${impl}`);
  console.log(`AIRACCOUNT_V0230_EXTENSION=${extension}`);
  console.log(`AIRACCOUNT_V0230_FACTORY=${factory}`);
  console.log(`AIRACCOUNT_V0230_AGENT_REGISTRY=${agentRegistry}`);
  console.log(`AIRACCOUNT_V0230_BLS_ALGORITHM=${blsAlgorithm}`);
  console.log(`AIRACCOUNT_V0230_BLS_AGGREGATOR=${blsAggregator}`);
  console.log(`AIRACCOUNT_V0230_VALIDATOR_ROUTER=${validatorRouter}`);
  console.log(`AIRACCOUNT_V0230_SESSION_KEY_VALIDATOR=${sessionKeyValidator}`);
  console.log(`AIRACCOUNT_V0230_FORCE_EXIT_MODULE=${forceExitModule}`);
  console.log(`AIRACCOUNT_V0230_DELEGATE=${delegate}`);
  console.log(`AIRACCOUNT_V0230_PARSER_REGISTRY=${parserRegistry}`);
}
