/**
 * e2e-v0.26.0.ts — Full on-chain E2E for v0.26.0 (isValidOwnerAuth, issue #159) on Sepolia.
 *
 ...
 * harness), PLUS the v0.26.0 additions:
 ...
 *            proves the rebase brought #158 into the v0.26.0 factory.
 *
 ...
 *
 * Run: pnpm tsx scripts/e2e-v0.26.0.ts
 */
...
const IMPL           = getAddress(process.env.AIRACCOUNT_V0260_IMPL!);
const VALIDATOR_ROUTER = getAddress(process.env.AIRACCOUNT_V0260_VALIDATOR_ROUTER!);
...
async function main() {
  ...
  // ── v0.26.0 security-hardening on-chain checks ──────────────────────────────
  ...
  try {
    const ROUTER = getAddress(process.env.AIRACCOUNT_V0260_VALIDATOR_ROUTER!);
    const SKV    = getAddress(process.env.AIRACCOUNT_V0260_SESSION_KEY_VALIDATOR!);
