/**
 * deploy-m7-r8.ts — Deploy AirAccount M7 r8 (review fixes) to Sepolia
 *
 * Changes from r7:
 *   - AAStarAirAccountFactoryV7: guardian dedup check in createAccount (MEDIUM)
 *   - AgentSessionKeyValidator: MAX_SELECTORS=30 cap (MEDIUM)
 *   - AAStarAirAccountV7: ModuleInstallCallbackFailed event (MEDIUM)
 *   - TierGuardHook: revert UnknownAlgId instead of silent return 0 (LOW)
 *   - Removed algId 0x09 from initConfig (was undefined, now reverts)
 *
 * Deploys:
 *   1. AirAccountCompositeValidator
 *   2. TierGuardHook
 *   3. AgentSessionKeyValidator
 *   4. AAStarAirAccountFactoryV7 (r8 implementation)
 *   5. Test account via factory (salt=1000, fresh account for E2E)
 *   6. Funds account with 0.05 ETH
 *
 * Usage:
 *   pnpm tsx scripts/deploy-m7-r8.ts
 *
...
const SALT = 1000n; // M7 r8 — fresh account for clean E2E
...
async function main() {
  ...
  console.log("=== Deploy AirAccount M7 r8 (review fixes) to Sepolia ===\n");
  ...
  console.log("\n[4/6] Deploy AAStarAirAccountFactoryV7 (M7 r8)...");
  ...
  const initConfig = {
    ...
    dailyLimit: 0n,
    // r8 fix: removed 0x09 (undefined algId, now reverts with UnknownAlgId)
  ...
  // ─── Summary ─────────────────────────────────────────────────────────────
  console.log("\n═══════════════════════════════════════════════════════════════");
  console.log(" M7 r8 Deployment Complete");
  console.log("═══════════════════════════════════════════════════════════════");
  console.log("\nAdd to .env.sepolia:");
  console.log(`AIRACCOUNT_M7_R8_FACTORY=${factoryAddr}`);
  console.log(`AIRACCOUNT_M7_R8_IMPL=${implAddr}`);
  console.log(`AIRACCOUNT_M7_R8_ACCOUNT=${predictedAddr}`);
  console.log(`AIRACCOUNT_M7_R8_COMPOSITE_VALIDATOR=${compositeValidatorAddr}`);
  console.log(`AIRACCOUNT_M7_R8_TIER_GUARD_HOOK=${tierGuardHookAddr}`);
  console.log(`AIRACCOUNT_M7_R8_AGENT_SESSION_VALIDATOR=${agentSessionValidatorAddr}`);
