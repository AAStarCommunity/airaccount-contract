/**
 * deploy-v0.20.2.ts — Patch deploy for v0.20.2 (P-256 mixed-sig module governance, issue #127).
 *
 * Both AAStarAirAccountV7 and AirAccountExtension changed:
 *   - installModule / uninstallModule moved from V7 → Extension
 *   - _checkGuardianSigsMixed added (ECDSA + P-256 mixed guardian sigs)
 ...
 * All other singletons are reused from v0.20.1.
 *
 * Reused from v0.20.1 (unchanged bytecode):
 ...
 *
 * Output env keys: AIRACCOUNT_V0202_*
 *
 * Usage: pnpm tsx scripts/deploy-v0.20.2.ts
 */
...
// ── Reused v0.20.1 singletons (must be set in .env.sepolia) ──────────────────
function v0201(key: string): Address {
  const v = process.env[`AIRACCOUNT_V0201_${key}`];
  if (!v) throw new Error(`AIRACCOUNT_V0201_${key} not set in .env.sepolia — run deploy-v0.20.1.ts first`);
  return getAddress(v);
}
...
async function main() {
  ...
  console.log(`\n=== v0.20.2 patch deploy — Sepolia ===`);
  console.log(`Deployer: ${deployer.address}\n`);

  // ── Reused v0.20.1 singletons ──────────────────────────────────────────
  const blsAlgorithm        = v0201("BLS_ALGORITHM");
  const validatorRouter     = v0201("VALIDATOR_ROUTER");
  const blsAggregator       = v0201("BLS_AGGREGATOR");
  const sessionKeyValidator = v0201("SESSION_KEY_VALIDATOR");
  const forceExitModule     = v0201("FORCE_EXIT_MODULE");
  const delegate            = v0201("DELEGATE");
  const parserRegistry      = v0201("PARSER_REGISTRY");
  console.log("Reusing v0.20.1 singletons:");
  ...
  // ── Deploy: impl (embeds new Extension with mixed-sig module governance) ─
  console.log("[1/3] AAStarAirAccountV7 (impl + AirAccountExtension v0.20.2)...");
  ...
  if (ver !== "0.20.2") throw new Error(`Version mismatch: expected "0.20.2", got "${ver}"`);
  console.log("  ✓ Version confirmed\n");

  // ── Summary ───────────────────────────────────────────────────────────────
  console.log("=== v0.20.2 Deployment Complete ===");
  console.log("Append to .env.sepolia:\n");
  console.log(`AIRACCOUNT_V0202_IMPL=${impl}`);
  console.log(`AIRACCOUNT_V0202_EXTENSION=${extension}`);
  console.log(`AIRACCOUNT_V0202_FACTORY=${factory}`);
  console.log(`AIRACCOUNT_V0202_AGENT_REGISTRY=${agentRegistry}`);
  console.log(`AIRACCOUNT_V0202_BLS_ALGORITHM=${blsAlgorithm}`);
  console.log(`AIRACCOUNT_V0202_BLS_AGGREGATOR=${blsAggregator}`);
  console.log(`AIRACCOUNT_V0202_VALIDATOR_ROUTER=${validatorRouter}`);
  console.log(`AIRACCOUNT_V0202_SESSION_KEY_VALIDATOR=${sessionKeyValidator}`);
  console.log(`AIRACCOUNT_V0202_FORCE_EXIT_MODULE=${forceExitModule}`);
  console.log(`AIRACCOUNT_V0202_DELEGATE=${delegate}`);
  console.log(`AIRACCOUNT_V0202_PARSER_REGISTRY=${parserRegistry}`);
}
