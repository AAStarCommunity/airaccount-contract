/**
 * e2e-v0.28.0.ts — Full on-chain E2E for v0.28.0 (isValidOwnerAuth, issue #159) on Sepolia.
 *
 ...
 * harness), PLUS the v0.28.0 additions:
 ...
 *            proves the rebase brought #158 into the v0.28.0 factory.
 *
 ...
 *
 * Run: pnpm tsx scripts/e2e-v0.28.0.ts
 */
...
const IMPL           = getAddress(process.env.AIRACCOUNT_V0280_IMPL!);
const VALIDATOR_ROUTER = getAddress(process.env.AIRACCOUNT_V0280_VALIDATOR_ROUTER!);
...
async function main() {
  ...
  // ── v0.28.0 security-hardening on-chain checks ──────────────────────────────
  ...
  try {
    const ROUTER = getAddress(process.env.AIRACCOUNT_V0280_VALIDATOR_ROUTER!);
    const SKV    = getAddress(process.env.AIRACCOUNT_V0280_SESSION_KEY_VALIDATOR!);
  ...
  // T31 (v0.28.0 DVT unification): router 0x01 == DVT validator, and the DVT validator validates a real
  ...
  try {
    const ROUTER = getAddress(process.env.AIRACCOUNT_V0280_VALIDATOR_ROUTER!);
    const DVT = getAddress(process.env.AIRACCOUNT_V0280_DVT_VALIDATOR!);
