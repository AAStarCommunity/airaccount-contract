/**
 * deploy-v0.25.0.ts — Deploy for v0.25.0 (CRITICAL-1 validation/execution tier desync fix).
 *
 * What changed vs v0.24.0: ONLY AAStarAirAccountBase (the M1 raw-65 fallback removed +
 * _deriveStoredAlgId single-source routing + revert-free router path). No validator-stack change.
 *
 * REUSES the entire v0.24.0 validator stack (router + SessionKeyValidator + BLS + aggregator +
 * ForceExit + Delegate + ParserRegistry). Deploys ONLY:
 *   - AAStarAirAccountV7 (new impl, same v0.24.0 router baked in) — brings a fresh AirAccountExtension
 *   - AAStarAirAccountFactoryV7 (new factory → new impl)
 *   - AgentRegistry (fresh — bindFactory is set-once, the v0.24.0 registry is bound to the old factory)
 *
 * Wiring: agentRegistry.bindFactory(factory); factory.setAgentRegistry(agentRegistry).
 *
 * NOTE (SDK coordination): removing raw-65 breaks aastar-sdk's Tier-1 tiering path
 * (bls-signature-service.ts generateTieredSignature(tier=1) emits raw 65-byte) — see aastar-sdk#273.
 * Existing accounts (old impl) are unaffected. New accounts need the SDK 0x02-prefix fix.
 *
 * Output env keys: AIRACCOUNT_V0250_*
 * Usage: pnpm tsx scripts/deploy-v0.25.0.ts
 */
...
function v0240(key: string): Address {
  const v = process.env[`AIRACCOUNT_V0240_${key}`];
  if (!v) throw new Error(`AIRACCOUNT_V0240_${key} not set in .env.sepolia — run deploy-v0.24.0.ts first`);
  return getAddress(v);
}
...
async function main() {
  ...
  console.log(`\n=== v0.25.0 deploy — Sepolia (CRITICAL-1 tier desync fix) ===`);
  console.log(`Deployer: ${deployer.address}\n`);

  // Reused v0.24.0 validator stack (unchanged bytecode).
  const router          = v0240("VALIDATOR_ROUTER");
  const sessionValidator = v0240("SESSION_KEY_VALIDATOR");
  const blsAlgorithm    = v0240("BLS_ALGORITHM");
  const blsAggregator   = v0240("BLS_AGGREGATOR");
  const forceExitModule = v0240("FORCE_EXIT_MODULE");
  const delegate        = v0240("DELEGATE");
  const parserRegistry  = v0240("PARSER_REGISTRY");
  console.log("Reusing v0.24.0 validator stack:");
  ...
  // [1] new impl (same v0.24.0 router baked in) → fresh Extension.
  console.log("[1/3] AAStarAirAccountV7 (impl, validatorRouter = reused v0.24.0 router)...");
  ...
  // [3] fresh AgentRegistry (bindFactory is set-once → the v0.24.0 registry is bound to the old factory).
  ...
  console.log("=== v0.25.0 Deployment Complete ===");
  console.log("Append to .env.sepolia:\n");
  console.log(`AIRACCOUNT_V0250_IMPL=${impl}`);
  console.log(`AIRACCOUNT_V0250_EXTENSION=${extension}`);
  console.log(`AIRACCOUNT_V0250_FACTORY=${factory}`);
  console.log(`AIRACCOUNT_V0250_AGENT_REGISTRY=${agentRegistry}`);
  console.log(`AIRACCOUNT_V0250_VALIDATOR_ROUTER=${router}`);
  console.log(`AIRACCOUNT_V0250_SESSION_KEY_VALIDATOR=${sessionValidator}`);
  console.log(`AIRACCOUNT_V0250_BLS_ALGORITHM=${blsAlgorithm}`);
  console.log(`AIRACCOUNT_V0250_BLS_AGGREGATOR=${blsAggregator}`);
  console.log(`AIRACCOUNT_V0250_FORCE_EXIT_MODULE=${forceExitModule}`);
  console.log(`AIRACCOUNT_V0250_DELEGATE=${delegate}`);
  console.log(`AIRACCOUNT_V0250_PARSER_REGISTRY=${parserRegistry}`);
}
