...
async function withRetry<T>(label: string, fn: (rpcUrl: string) => Promise<T>): Promise<T> {
  for (const rpcUrl of RPC_URLS) {
    try {
      ...
      console.warn(`  [${label}] Failed on ${rpcUrl.slice(0, 60)}: ${e.message?.slice(0, 80)}`);
    }
  }
...
async function main() {
  ...
  const predicted = await withRetry("getAddress", async (rpcUrl) => {
    const pub = createPublicClient({ chain: sepolia, transport: http(rpcUrl, { timeout: 120_000 }) });
  ...
  const code = await withRetry("getBytecode", async (rpcUrl) => {
    const pub = createPublicClient({ chain: sepolia, transport: http(rpcUrl, { timeout: 120_000 }) });
    return pub.getBytecode({ address: predicted });
  });

  if (code && code.length > 2) {
    console.log("Already deployed — skipping createAccount");
  } else {
    await withRetry("createAccount", async (rpcUrl) => {
      const pub = createPublicClient({ chain: sepolia, transport: http(rpcUrl, { timeout: 120_000 }) });
      const wal = createWalletClient({ account: owner, chain: sepolia, transport: http(rpcUrl, { timeout: 120_000 }) });
      ...
      await waitTx(pub, h, "createAccount");
    });
  }

  // Fund account
  await withRetry("fund", async (rpcUrl) => {
    const pub = createPublicClient({ chain: sepolia, transport: http(rpcUrl, { timeout: 120_000 }) });
    const wal = createWalletClient({ account: owner, chain: sepolia, transport: http(rpcUrl, { timeout: 120_000 }) });
  ...
  // EP deposit
  await withRetry("depositToEntryPoint", async (rpcUrl) => {
    const pub = createPublicClient({ chain: sepolia, transport: http(rpcUrl, { timeout: 120_000 }) });
    const wal = createWalletClient({ account: owner, chain: sepolia, transport: http(rpcUrl, { timeout: 120_000 }) });
