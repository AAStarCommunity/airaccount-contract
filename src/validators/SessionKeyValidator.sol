...
contract SessionKeyValidator is IAAStarAlgorithm {
    ...
    // ─── Backward-compat shims (TEMPORARY — removed before v0.17.2 final tag) ──
    // These let existing tests compile while we incrementally migrate them to the new
    // Session-struct API. Internally they build a Session with empty arrays + zero velocity,
    // which is semantically equivalent to the old "simple session" behaviour.
    ...
    function grantSession(
        ...
        _validateCfgMem(cfg);
        _checkNotExists(account, sessionKey);
        bytes32 grantHash = _buildGrantHashMem(account, sessionKey, cfg);
        address recovered = grantHash.recover(ownerSig);
        if (recovered != _ownerOf(account)) revert NotAccountOwner();
        _sessions[account][sessionKey] = cfg;
        emit SessionGranted(account, sessionKey, cfg);
    ...
    /// @dev Backward-compat: old auto-getter shape (4 fields). New code should use getSession().
    function sessions(address account, address sessionKey)
        external view
        returns (uint48 expiry, address contractScope, bytes4 selectorScope, bool revoked)
    {
        Session storage s = _sessions[account][sessionKey];
        return (s.expiry, s.contractScope, s.selectorScope, s.revoked);
    }

    /// @dev Backward-compat: old auto-getter shape for P256.
    function sessions_p256(address account, bytes32 keyHash)
        external view
        returns (uint48 expiry, address contractScope, bytes4 selectorScope, bool revoked)
    {
        Session storage s = _sessions_p256[account][keyHash];
    ...
    /// @dev Backward-compat: old buildGrantHash with 5 args (no velocity/arrays).
    function buildGrantHash(
        ...
        return _buildGrantHashMem(account, sessionKey, cfg);
    }
    ...
    function buildP256GrantHash(
        ...
        return _buildP256GrantHashMem(account, keyX, keyY, cfg);
    }

    function _validateCfgMem(Session memory cfg) internal view {
    ...
    function _buildGrantHashMem(address account, address sessionKey, Session memory cfg)
