...
$100 是发行价 $0.02 的 **5,000×**（整段带宽 MAX/MIN = 100,000×；按单次 +30% 走完约 **32–33 步**，
非 40 步）。于是固定的代币数量在带宽两端是这样：
...
> **来源标注**：价格带宽与「新代币从 0 开始」两条均为 @repo:sp 转述其仓库代码，本仓无可验落点，
> 未独立核实；上表的换算与 5,000×/32–33 步是本仓自算。已上链独立核实的只有：线上 2.1.0 工厂的
> `aPNTsPriceUSD = 2e16`、其 `owner()` 为 EOA `0x51Ac6949…`、旧代币 `totalSupply = 140,000e18`
> 且本仓两个地址余额为 0。
