...
📌 **本仓能核到的唯一真实余额**:在本仓实际接线的 Sepolia aPNTs
`0xDf669834…`（`totalSupply` = 378,709.7）上,Jason EOA `0xb5600060…` 持有 **2,780 aPNTs**;
CC-31 Safe 与两个 e2e 账户均为 0。**2,780 落在上表第二行**——若这是典型量级,
tier-2/tier-3 实际上永不触发,整个钱包等于单签。
（@repo:sp 引用的是另一个 Sepolia 地址上的 15,539,该地址本仓未能核实,见 CC-46 往来。）
