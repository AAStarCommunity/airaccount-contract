...
⚠️ **给铸币计划（C）的输入,不是阻塞项**:按**旧**代币 140,000 的盘子量,
`standard` 的 daily（150,000）是供应量的 1.1×、`trader` 的 daily（750,000）是 5.4×——
**这两档的 daily 上限会形同虚设**（tier1/tier2 仍然生效,因为 1,500 / 15,000 远低于盘子）。
所以铸币量若落在 140,000 量级,应知道 daily 这一路的 tier-3 升级对这两档不会触发。
