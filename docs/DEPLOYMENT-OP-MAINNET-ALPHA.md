...
| profile | tier1/tier2/daily (aPNTs) | = 美元 | 占总供应量 |
|---|---|---|---|
| conservative | 100 / 1,000 / 5,000 | $2 / $20 / $100 | 0.1% / 0.7% / **3.6%** |
| standard | 500 / 5,000 / 10,000 | $10 / $100 / $200 | 0.4% / 3.6% / **7.1%** |
| trader | 2,000 / 20,000 / 50,000 | $40 / $400 / $1,000 | 1.4% / **14.3%** / **35.7%** |

⇒ **conservative 档合理；trader 档不合理**——单账户单日可动用全部流通盘的 **35.7%**，
tier-2 单笔 14.3%，这作为单账户限额说不通，与价格无关。

**需要 Jason 定的是一件事，不是一个数字**：aPNTs 的 guard 到底是
**（A）价值闸门**（按美元，则本仓必须承认它对 aPNTs 不可实现，写进 known-issues）还是
**（B）供应量比例闸门**（按占比设档，与 $ 脱钩，则现有 conservative 可留、trader 必须调小）。
**不能两者兼得**，而且烘焙后 tier1/tier2 **永久不可调**。本仓倾向 (B)。
在此决定之前不要烘焙 aPNTs 配置——反正 P5 的 B1/B2 也还没解除，不阻塞任何事。
