...
> ## ⚠️ v0.17.1 integrators — use the merged full ABI
>
> As of v0.17.1 the account is **diamond-lite**: the cold ERC-8004 agent functions
> (`setAgentWallet`, `mintAgentIdentity`, `bindERC8004AgentWallet`, `submitAgentReputation`,
> `queryAgentReputation`) and the weighted-signature config-governance functions
> (`setWeightConfig`, `proposeWeightChange`, `approveWeightChange`, `executeWeightChange`,
> `cancelWeightChange`) live in `AirAccountExtension` and are reached through the account's
> `fallback`+`delegatecall`. They are **not** in the compiled `AAStarAirAccountV7` ABI.
>
> **Always bind to [`abi/AAStarAirAccountV7.full.json`](../abi/AAStarAirAccountV7.full.json)** — the
> merged ABI (V7 + `IAirAccountAgent`) generated by `scripts/build-full-abi.mjs`. Binding to the raw
> `out/` artifact will make these 10 functions un-callable (no selector in the ABI). All addresses
> below are **release-pending** for v0.17.1 — do not assume a pre-M7 address serves the agent/diamond-lite
> code path (see [contract-registry §3](./contract-registry.md#3-deployed-addresses-sepolia)).
