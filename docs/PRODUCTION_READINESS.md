...
| 3 | **切一个 tagged release**（bump `FACTORY_VERSION` + `[Unreleased]`→版本号） | 当前常量 0.27.0；改名等改动在 `[Unreleased]` | 🟡 待发版 | 是 | [测]→[主] |
| 4 | **外部安全审计（#29）** | 未做。非可升级钱包托管资金，GA 硬门禁 | 🔴 未开始（jason 安排） | **是（GA 阻塞）** | [主] |
| 5 | **Etherscan verify** | key 已有（`~/Dev/.env` `1JY…` / `.env.sepolia` `MZD…`）；Sepolia **2/11 verified，9 pending**（foundry source path 问题）；主网 verify 走 chainid 10 | 🟠 未完成（工具路径问题待修） | 是（透明度/信任） | [测]+[主] |
| 6 | **主网合约部署** | 未开始；`deploy-op-mainnet-alpha.ts` 待创建（基于 deploy-v0.20/v0.27 改造 + cast wallet 签名） | 🔴 未开始 | 是 | [主]+[配] |
